function out = filter_boundary_single(fil_num,fil_den1,fil_den2,in,condition)
%#codegen

% condition 0 == 'normal'
% condition 1 == 'transpose'
...
if condition  % transpose==1, normal==0
    tmp_out = flipud(filter(fil_num,1,flipud(in)));
else
    tmp_out = filter(fil_num,1,in);
end
% out = out_full(shift+5+1:end-shift-5);

out = zeros(length(in),1, 'single');

% denominator
%if(length(fil_den)==2)
    causal = fil_den1; %causal part
    non_causal = fil_den2;  %noncausal part
    %     disp('in1')
    if (length(causal)+length(non_causal)>2) % min(length(causal,noncausal)) =1, so if sum less than 2, no need to filter.
        ...
        tmp_out = [zeros(shiftnc,1);tmp_out;zeros(shiftnc,1)];
%         if shiftnc~=0
%             tmp = zeros(shiftnc*2+length(tmp),1);
%             tmp(2:end-1,1)=out;
%             out=tmp;
%             
%         end
        ...
        %         disp('in2')
        if(condition == 0)
            tmp_out = filter(1,causal,tmp_out); %causal
            tmp_out = flipud(filter(1,non_causal,flipud(tmp_out)))*(non_causal(end)); %ncausal
            out = tmp_out(2*shiftnc+1:end);
            %             disp('in3')
        elseif(condition == 1);
            tmp_out = flipud(filter(1,causal,flipud(tmp_out)));
            tmp_out = filter(1,non_causal,tmp_out)*(non_causal(end));
            out = tmp_out(1:end-2*shiftnc);
            %             disp('in4')
        end
    end
%end
